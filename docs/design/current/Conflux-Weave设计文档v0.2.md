...
模块拆分的目标不是让文件或函数越小越好，而是让职责边界更准确、变更半径更可控。后续开发遵循以下顺序：

1. **内聚优先**：一起变化且专属的组件、私有子组件、类型和 hook 放在同一文件；不要为了行数制造跨文件跳转。
2. **越过阈值才拆**：仅当逻辑被至少两处复用、能够独立测试或懒加载、一个文件包含两个互不调用的概念，或修改一处反复担心误伤另一处/造成上下文过载时才拆分。
3. **按职责域拆**：以超时检测、数据库同步、报告生成等稳定逻辑边界拆分，而不是按固定行数切割。
4. **自然边界**：使用 AST/tree-sitter 按函数、类和类型边界提取，禁止在任意语句中间切断职责。
5. **薄壳模式**：原有公共入口和兼容路径保留为薄壳，只负责导入、组装和转发；核心逻辑进入职责域模块。

拆分完成后必须保持公开导入路径、状态机合同、Artifact lineage 和测试可复现性不变；若拆分会改变这些合同，应先形成独立决策记录，不在普通重构中隐式吸收。
